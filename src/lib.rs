...
fn un_pad(data: Vec<u8>) -> Vec<u8> {
    let pad_len = data.last().unwrap_or(&0);
    let data_len = data.len() - *pad_len as usize;
    let unpaded_data = data.as_slice()[0..data_len].to_vec();
    unpaded_data
}
...
#[cfg(test)]
mod test {
    use crate::{pad, un_pad};

    #[test]
    fn test_unpad() {
        let data = [1, 2, 3];
        let paded_data = pad(data.to_vec());
        let unpaded_data = un_pad(paded_data);

        assert_eq!(data, unpaded_data.as_slice());
    }
}
